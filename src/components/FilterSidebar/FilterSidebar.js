...
import { CATEGORIES, FILE_FORMATS, PRICE_RANGE, POLY_COUNT_RANGE } from '../../data/products';
  ...
  return (
    <div className="range-slider" role="group" aria-labelledby={`${id}-label`}>
      ...
      <div className="range-slider__values">
        <span className="range-slider__value">{formatValue(localValue[0])}</span>
        <span className="range-slider__separator" aria-hidden="true" />
        <span className="range-slider__value">{formatValue(localValue[1])}</span>
      </div>
    </div>
  );
});
...
const FilterSidebar = memo(function FilterSidebar({
  ...
  const handlePolyCountChange = useCallback((polyCountRange) => {
  ...
  return (
    <>
      ...
      >
        ...
        {/* Decorative Elements */}
        <div className="filter-sidebar__decor filter-sidebar__decor--top" aria-hidden="true" />
        <div className="filter-sidebar__decor filter-sidebar__decor--bottom" aria-hidden="true" />
      </aside>
    </>
  );
});
